...
import {  YoutubeOutlined, InstagramOutlined, LinkedinOutlined, FacebookOutlined, DesktopOutlined, IdcardOutlined, BankOutlined, FormOutlined, OrderedListOutlined, ScheduleOutlined, CalendarOutlined, SettingOutlined, BookOutlined, CarOutlined, CodeOutlined, ToolOutlined, CompassOutlined } from '@ant-design/icons';
import { Breadcrumb, theme, Menu, Layout, Table, Avatar, Input, Select } from 'antd';
import { useState } from 'react';
import ActionColumn from './Student/Helpers/ActionColumn';
...
function SchoolApp() {
  ...
  const FillterhandleChange = (e) => {
    console.log("******&&&&&&&&", e)
    const asa = first_name.filter((res) => res.first_name === e.target.value)
    console.log("@@@@@@@@@@@@ name", asa)
    setData(asa)
  }
...
// }
  ...
  const columns = [
...

    {
      ...
      width: '50px',
      render: (record) => {
        return <ActionColumn record={record} eventCb={columnEventCb} user={data}/>
      }
    },
  ];
          ...
          <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', margin: '20px' }}>
            ...
            <Space style={{ marginBottom: '12px' }} >
                <Select onSelect={(event) => FillterhandleChange(event)} defaultValue="All Your Choice" >
                  <Option value="Mauni" >M</Option>
                  <Option value="Naina" >N</Option>
                </Select>
              </Space>
            {/* <Space style={{ marginBottom: '12px' }} > */}
            {/* <Select defaultValue="All Your Choice" >
